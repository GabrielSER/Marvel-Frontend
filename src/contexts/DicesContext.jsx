...
const DicesProvider = (props) => {
    ...
    const updateTotal = useCallback((result) => {
    ...
    const addRoll = useCallback((rollComponent) => {
        setState({
            ...state,
    ...
    const removeRoll = useCallback((key) => {
    ...
    return <DicesContext.Provider value={value} {...props} />
}

const useDices = () => {
    const context = useContext(DicesContext)
    if (!context) {
        throw new Error('Invalid use of useDices, DicesProvider must be defined in parent hierarchy')
    }
    return context
}
